...
import 'package:dima_project/im/user_profile.dart';
...
class _ChatState extends State<Chat> {
  ...
  _onTapAvatar(
      String userID, TapDownDetails tapDetails, bool isCustomerServiceChat) {
    final isWideScreen =
  ...
  _buildBottomOperationList(BuildContext context,
      V2TimConversation conversation, VoidCallback closeFunc) {
    List operationList = [
      {
        "label": TIM_t("发送消息"),
        "id": "sendMsg",
    ...
    return operationList.map((e) {
      return TIMUIKitProfileWidget.wideButton(
        ...
        text: e["label"] ?? "",
        color: hexToColor("3e4b67"),
      );
    }).toList();
  }

  void onClickUserName(Offset offset, bool isCustomerServiceChat,
      [String? user]) {
                  ...
                  profileWidgetBuilder: ProfileWidgetBuilder(customBuilderOne:
                    ...
                    return Column(
                        children: _buildBottomOperationList(
                            context, conversation, closeFunc));
                  }),
                  profileWidgetsOrder: [
  ...
  Widget build(BuildContext context) {
    ...
    return TIMUIKitChat(
      ...
      lifeCycle:
        ...
        //     });
        //   }
        //   if (TencentCloudChatCustomerServicePlugin.isCanSendEvaluateMessage(message) &&
        //       !TencentCloudChatCustomerServicePlugin.isCanSendEvaluate(message) &&
        //       canSendEvaluate == true) {
        //     setState(() {
        //       canSendEvaluate = false;
        //     });
        //   } else if (TencentCloudChatCustomerServicePlugin.isCanSendEvaluateMessage(message) &&
        //       TencentCloudChatCustomerServicePlugin.isCanSendEvaluate(message) &&
        //       canSendEvaluate == false) {
        //     setState(() {
        //       canSendEvaluate = true;
        ...
        // if (TencentCloudChatCustomerServicePlugin.isCustomerServiceMessageInvisible(message) &&
        //     TencentCloudChatCustomerServicePlugin.isCustomerServiceMessage(message)) {
        //   return false;
        // }
        return true;
      ...
      onDealWithGroupApplication: (String groupId) {
        if (!isWideScreen) {
          ...
          TUIKitWidePopup.showPopupWindow(
            ...
            title: TIM_t("Group Join Requests"),
      ...
      onTapAvatar: (userID, tapDetails) =>
          _onTapAvatar(userID, tapDetails, isCustomerServiceChat),
